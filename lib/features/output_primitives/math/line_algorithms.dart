...
/// Pure static mathematical utility for generating line coordinates.
/// Zero UI or GetX dependencies.
class LineAlgorithms {
  static List<Offset> generateLinePoints(LineAlgorithm algo, Offset start, Offset end) {
    if (algo == LineAlgorithm.dda) {
      return generateDdaLine(start, end);
    } else {
      return generateBresenhamLine(start, end);
    }
  }

  /// Generates raster points for a line using the DDA algorithm.
  ...
  /// Generates raster points for a line using Bresenham's algorithm.
